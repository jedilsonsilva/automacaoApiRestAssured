...
//VALIDAÇÕES DOS STATUS CODE DE ERRO
    ...
    @Severity(SeverityLevel.NORMAL)
    @Category({Healthcheck.class, AllTests.class})
    @DisplayName("405 - Validar o status code informando um método não suportado na obtenção de dados da conta consentida")
    public void testMetodoNaoSuportadoContas() throws Exception {
        getContaRequest.metodoNaoSuportadoContas()
                .then()
                .log().all()
                .statusCode(405)
