...
public class GetConsentimentoPagamentoTest extends BaseTest {
    ...
    public void testGarantirContratosConsultaConsentimento() throws Exception {
        getConsentimentoPagamentoRequestRequest.validarContratoConsultaConsentimentoPagamentoPix()
                .then()
                .statusCode(200)
                .assertThat().body(matchesJsonSchema(
                new File(Utils.getContractsBasePath("fase03/Pagamentos/consentimentoPagamentoPix", "GetConsentimentoPagamentoPix"))));
    }
    @Test
    @Severity(SeverityLevel.NORMAL)
   ...
   /* @Test
    ...
    public void testRecursoInexistente() throws Exception {
        getConsentimentoPagamentoRequestRequest.recursoInexistente()
                ...
                .time(lessThan(4L), TimeUnit.SECONDS)
                .body("errors.title", hasItem("O recurso solicitado não existe."))
                .body("errors.detail", hasItem("Nenhum consentimento encontrado para o consentId urn:realizecfi:f5428c42-11ac-422d-a7c3-e57b."));
    }
    @Test
    @Severity(SeverityLevel.NORMAL)
    @Category({Healthcheck.class, AllTests.class, fase03.class})
